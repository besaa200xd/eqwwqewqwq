...
if(!message.member.voice.channel) return message.channel.send({embed: {color: FynxHata, description: `Ses seviyesini ayarlabilmek için bir ses kanalında olmanız gerekmektedir!` }})  
...
if (!sesseviyesi) return message.channel.send({embed: {color: FynxHata, description: `Ses seviyesini ayarlayabilmek için sadece bir doğal sayı giriniz.` }})
...
message.channel.send({embed: {color: FynxDogru, description: `Ses seviyesi, başarılı bir şekilde \`${args.join(" ")}\` olarak ayarlandı! ` }})
}
